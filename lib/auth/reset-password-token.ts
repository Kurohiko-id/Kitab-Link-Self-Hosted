...
// server yang sah. Disimpen in-memory per email (bukan tabel DB) -- short-lived (15 menit)
...
export function generateResetToken(email: string): string {
  ...
  tokens.set(email, { token, expires: Date.now() + TTL_MS });
  console.log(`[kitab-link] Password reset token buat ${email} (berlaku 15 menit): ${token}`);
  return token;
}
